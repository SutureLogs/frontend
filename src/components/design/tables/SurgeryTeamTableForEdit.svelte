    ...
    <div class="flex justify-end mt-4 gap-2">
      ...
      <LinkButton
        ...
        onClick={async () => {
          ...
          if (
            data.find(
              (member) => member.doctorusername === currentMember.doctorusername
            )
          ) {
          ...
          const response = await axios.get(
            BASEURL +
              `/doctor/username-exists?username=${currentMember.doctorusername}`
          );
          ...
          data = [
            ...data,
            {
              doctorusername: currentMember.doctorusername,
      ...
      {#each data as member, i}
        <tr>
          ...
          <td class="flex gap-4 items-center">
            ...
            <div
              on:click={() => {
                if ($store.username === member.doctorusername) {
                  toast.error("You cannot remove yourself");
                  return;
                }
                data = data.filter((m) => {
                  return m.doctorusername !== member.doctorusername;
                });
              }}
            >
              <svg
